...
public:
  ...
  void Begin(NeoPixelType type, uint16_t pixelCount)
  {
    ...
    NeoScreen_Lines_Type lineType = NeoScreen_Lines_Cols;
    size_t numLines = 22;
  ...
  void SetPixelColorRaw(uint16_t matrixPixel, RgbwColor c)
  {
    uint16_t indexPixel = _pixelMap[matrixPixel];
    if(UINT16_MAX == indexPixel) {
      return;
    }
  ...
  RgbwColor GetPixelColorRaw(uint16_t matrixPixel) const
  {
    uint16_t indexPixel = _pixelMap[matrixPixel];
    if(UINT16_MAX == indexPixel) {
      return pGrb0->GetPixelColor(0);
    }
